...
// Test Hospital
    {
        ...
        cout << "1 Admit: " << h.admit_patient(p1) << endl;
        cout << "2 Admit: " << h.admit_patient(p1) << endl;
        cout << h << endl;
        cout << "3 Admit: " << h.admit_patient(p2) << endl;
        cout << "4 Admit: " << h.admit_patient(p3) << endl;
        cout << h << endl;
        p2 = nullptr;
        cout << h << endl;
        cout << "5 Admit: " << h.admit_patient(p3) << endl;
        cout << h << endl;
    ...
    }
    {
        const vector meds{Medical_Specialty::Cardiology, Medical_Specialty::Dermatology, Medical_Specialty::Endocrinology, Medical_Specialty::Gynecology, Medical_Specialty::Neurology, Medical_Specialty::Oncology, Medical_Specialty::Pathology, Medical_Specialty::Pediatrics, Medical_Specialty::Pulmonology, Medical_Specialty::Urology};
        Hospital h("name");
        shared_ptr<Health_Care_Provider> s1{std::make_shared<Smart_Health_Care_Provider>(100,"Smart 1",std::set<Medical_Specialty>{meds[0],meds[6]},100)};
        shared_ptr<Health_Care_Provider> s2{std::make_shared<Smart_Health_Care_Provider>(100,"Smart 2",std::set<Medical_Specialty>{meds[1],meds[2]},2000)};
        shared_ptr<Health_Care_Provider> s3{std::make_shared<Smart_Health_Care_Provider>(100,"Smart 3",std::set<Medical_Specialty>{meds[0],meds[6]},100)};
        shared_ptr<Health_Care_Provider> s4{std::make_shared<Smart_Health_Care_Provider>(100,"Smart 4",std::set<Medical_Specialty>{meds[1],meds[2]},2000)};
        h.sign_hcp(s1);
        h.sign_hcp(s2);
        h.sign_hcp(s3);
        h.sign_hcp(s4);
        s1->increase_wealth(100);
        s2->increase_wealth(42);
        s3->increase_wealth(500);
        s4->increase_wealth(420);
        std::cout << std::endl;
        // for(const auto& x : h.getTopTurnover()) {
        //     std::cout << *(x.first.lock()) << ' ' << x.second << std::endl;
        // }
    }
