...
bool Teaching_Health_Care_Provider::teach(Medical_Specialty m, std::shared_ptr<Health_Care_Provider> target)
{
	...
	this->Health_Care_Provider::increase_wealth(fee);
	//target->Health_Care_Provider::pay_license(fee);	//dont need to cuz he alerady pays in if()
	target->receive_license(m); // target->receive_license(m); doesnt work 4 some reason //nevermind it works :P
	return true;
}
